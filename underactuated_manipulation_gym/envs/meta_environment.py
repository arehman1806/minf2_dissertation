...
class MetaEnvironmentv1(BaseEnvironment):

    def __init__(self, config_file):
        super(MetaEnvironmentv1, self).__init__(config_file)
        ...
        self.max_episode_steps = self._episode_length
    ...
    def _reward(self, observation, proprioception_indices, action):
        ...
        reward += 0.1 * (self.previous_distance - distance)
        self.previous_distance = distance
        
        # done 
        done = distance < self._environment_config["target_radius"]
    ...
    def step(self, action):
        ...
        observation[-2:] = np.array([action, success_policy], dtype=np.float32)
    ...
    def get_observation(self):
        ...
        observation_indices["polar_robot_object"]  = len(vect_obs) + 2
        # add polar coordinates to vector observation
        vect_obs = np.append(vect_obs, [object_target_polar_r, object_target_polar_theta, robot_object_polar_r, robot_object_polar_theta])
    ...
    def _get_observation_space(self):
        obs_space_dry_run = self.get_observation()[0]
        vect_obs_size = obs_space_dry_run.shape
